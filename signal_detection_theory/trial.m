function inputs = trial(h, dist)
    ...
    for ii = 1:10 %Filling temp1
        ...
        temp(200,200+dist,:) = (ii-1)*10; %Inserting a test point dist pixels right of it. Brightness range 0 to 90.
    ...
    inputs = ["Brightness" "Input" "Confidence"];
    ...
    for ii = 1:200 %200 trials, 20 per condition
        ...
        if temp2 == '.'; anse = 'Present'; else anse = 'Not Present'; end
        anse
        ...
        pause;
        conf = get(h,'CurrentCharacter')
        inputs(end+1,:) = [(stimulusorder(1,ii)-1)*10 anse string(conf)]
