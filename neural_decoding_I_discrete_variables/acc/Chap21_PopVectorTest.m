...
function correct = Chap21_PopVectorTest(i)
...
% Train/Test Split
split_from = i+1;
instruction = instruction(split_from:length(instruction));
go = go(split_from:length(go));
direction = direction(split_from:length(direction));
